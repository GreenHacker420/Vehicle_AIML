...
const ShaderBackground = ({ className }: ShaderBackgroundProps) => {
  ...
  const fsSource = `
    ...
    const vec4 lineColor = vec4(0.18, 0.76, 0.98, 1.0);
    ...
    void main() {
      ...
      vec4 lines = vec4(0.0);
      vec4 bgColor1 = vec4(0.035, 0.055, 0.14, 1.0);
      vec4 bgColor2 = vec4(0.08, 0.12, 0.24, 1.0);
  ...
  return (
    <canvas
      ref={canvasRef}
      className={cn("fixed inset-0 h-full w-full -z-20", className)}
    />
  );
};
